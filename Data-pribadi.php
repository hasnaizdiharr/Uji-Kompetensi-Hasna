...
<div class="d-md-flex justify-content-md-start p-2 ms-auto">
    <form method="GET" action="" style="display: inline;">
        <input type="text" name="searchNIK" placeholder="Cari NIK" style="padding: 5px; width: 200px;" value="<?php echo htmlspecialchars($searchNIK); ?>">
        <button type="submit" class="btn btn-custom">Cari</button>
    </form>
</div>
<nav>
<a href="form-daftar.php" style="display: inline-block; padding: 5px 8px; background-color:rgb(128, 146, 188); color: white; text-decoration: none; border-radius: 5px;">
  [+] Tambah Baru
</a>
</nav>
<br>
...
while($karyawan = mysqli_fetch_array($query)){
    ...
    echo "<td>".$karyawan['No_Rek']."</td>";
    echo "<td>";
    echo "<a class='btn btn-warning' href='form-edit.php?NIK=".$karyawan['NIK']."'>Edit</a> | ";
    echo "<a class='btn btn-danger' href='hapus-datapribadi.php?NIK=".$karyawan['NIK']."' onclick='return confirm(\"Anda Yakin ingin menghapus data ini?\")'>Hapus</a>";
    echo "</td>";
    echo "</tr>";
}
?>
